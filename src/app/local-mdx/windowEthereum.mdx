...
보안적 관점에서 보면 `if (window.ethereum)` 체크가 필요한 이유가 명확해진다.
...
window.ethereum은 브라우저가 원래 제공하는 기능이 아니다. <br />
EIP-1193 표준에 맞춰 지갑이 주입한 provider이며, DApp ↔ 지갑 ↔ 블록체인 사이의 전용 인터페이스다.

튜토리얼과 예제 코드가 `if (window.ethereum)`으로 시작하는 건 단순한 관례가 아니다. <br />
지갑이 없으면 연결 자체가 불가능하고, 지갑이 있어야만 블록체인 관련 로직을 실행할 수 있다.
