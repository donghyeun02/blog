...
AWS는 대부분 서비스가 관리형이다.
그만큼 안정성과 편리함이 크지만, 비싸다.

내 서비스 규모엔 상당한 오버스펙이었고 비용도 훨씬 많이 나왔다.
...
설계 하나가 비용을 만든다는 걸 직접 경험으로 배웠다.

---

트래픽도 없는데 왜 32달러가 나왔는지, 구조를 뜯어보고 나서야 이해됐다.

NAT Gateway는 존재 자체로 과금된다. Lambda가 외부 API를 한 번 부를 때마다 그 경로 위에 있는 NAT Gateway가 돈을 가져간다. 설계가 비용을 만든다는 걸 직접 경험으로 배웠다.
